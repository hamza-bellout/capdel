...
/**
 * Provides base functionality for the Menu Adder Queue Workers.
 *
 * @QueueWorker(
 *   id = "cron_menu_adder",
 *   title = @Translation("Cron Menu Adder"),
 *   cron = {"time" = 60}
 * )
 */
class ConfEntityRefProcess extends QueueWorkerBase implements ContainerFactoryPluginInterface {
    /**
     * The node storage.
     *
    ...
    public function processItem($data) {
        ...
        $query->condition('field_conf_lieu_ref_par_menu.entity.field_par_ref_statics.target_id',$data->id);
                ...
                if($loadedNode) {
                    ...
                    $referencedPlaces = $loadedNode->get('field_ref_lieu')->getValue();
                    ...
                    if(!$this->arrayCompare($referencedPlaces,$entity_ids)) {
                        $loadedNode->set('field_ref_lieu',$entity_ids);
                        $loadedNode->save();
